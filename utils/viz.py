...
def display_result(image: torch.Tensor, output: List[torch.tensor], target: torch.Tensor, file_path=None) -> None:
    _, ax = plt.subplots()
...
def display_with_bboxes(input: torch.Tensor, output: List[torch.tensor], multiple_classes=False) -> None:
    _, ax = plt.subplots()

    pad = 20
    image = input.numpy()[0,:].transpose(1,2,0)
    image = np.pad(image, ((pad, pad), (pad, pad), (0, 0)), mode='constant')
    ax.imshow(image)
    ...
    if output:
        ...
        for i in range(bboxes.shape[1]):
            ...
            # only show person
            if multiple_classes and int(bboxes[0,i,5]) != 14:
                continue
            ...
            if bboxes[0,i,-1] >= 0:
                ...
                rect = patches.Rectangle((cx,cy), w, h, linewidth=2, facecolor='none', edgecolor='r')
                ax.add_patch(rect)
                ax.annotate(f"{float(bboxes[0,i,4]):.2f}",(cx,cy), color='r')
