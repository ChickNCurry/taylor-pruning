...
class Camera(traitlets.HasTraits):
    ...
    capture_fps = traitlets.Integer(default_value=60)
...
class CameraDisplay:
    def __init__(self, img_to_display_img_callback, lazy_camera_init: bool = False, show_widget: bool = True):
    ...
    def initialize_camera(self):
        ...
        self.camera = Camera(width=640, height=360, capture_width=1280, capture_height=720, capture_fps=60)
...
class CameraDetectionDisplay(CameraDisplay):
    def __init__(self, net, img_to_display_img_callback, lazy_camera_init: bool = False, show_widget: bool = True):
        super(CameraDetectionDisplay, self).__init__(img_to_display_img_callback, lazy_camera_init, show_widget)
        self.net = net
        self.img_to_display_img_callback = img_to_display_img_callback
        self.show_widget = show_widget
    ...
    def _camera_callback(self, change):
        if not self._processing_frame:
            ...
            image = change['new']
            if not self.img_to_display_img_callback is None:
                image = self.img_to_display_img_callback(self.net, image)
            if self.show_widget:
                self.image_widget.value = bgr8_to_jpeg(image)
            self._processing_frame = False
